...
void Scene_Item_Ex::UpdateCategoryWindow() {
	...
	for (int i = category_min; i <= category_max; i++) {
		...
		if (i == category_index) {
			category_window->AddText(ToString(Main_Data::game_strings->Get(offset_category_strings + i)), Font::ColorCritical);
		}
		else {
			category_window->AddText(ToString(Main_Data::game_strings->Get(offset_category_strings + i)));
		}
		category_window->AddText("   ");
	}
}
